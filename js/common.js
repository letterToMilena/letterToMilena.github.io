$(document).ready(function(){
      ...
      var loadStart = 0;
      var loadEnd = 0;
      var loadValue = 0; //для сбора значений нагрузок;
      var shift; //переменная сдвига
    ...
    $("#f_answer, #f_fluid,#f_anchorage, #f_fixed, #f_force, #f_load, #f_moment").submit(function(){return false;});
    $('input#f_answer_input,input#f_dist_fluid, input#f_dist_fixed, input#f_force_value, input#f_dist_force, input#f_load_value, input#f_dist_start, input#f_dist_end, input#f_dist_moment, input#f_moment_value').unbind().blur( function(){
         ...
         switch(id)
       {
             case 'f_answer_input':
                ...
                if(val == '02112018' && val != '' && regExpLength.test(val))
                {
                   ...
                   $(this).next('.error-box').text('Ты права, Милена. Теперь ты знаешь кто я)')
                                             .css('color','green')
...
visiblePost = () => {
       if (document.getElementById('answer').getElementsByClassName('not_error').length<1){
        ...
        post.innerHTML+=`<br>Да, да. Это я, Алексей, твой недавний знакомый. Вот, решился на авантюру отыскать тебя и написать всё сюда. 
        У тебя появится вполне логичный вопрос, почему же таким странным способом, через Интернет? На самом деле все просто, я хоть и верю в свой успех, но не хочу чтобы бумажное письмо попало кому-нибудь, кроме тебя.
         Хотя очень трудно писать письмо, Милена, ведь я тебя почти не знаю, но я верю, что в данном случае перспектива снова встретиться и узнать тебя больше 
         послужит мне достаточным извинением. И к тому же, кроме как это письмо, у меня нет других шансов связаться с тобой.
         Надеюсь, удача будет на моей стороне и ты сможешь его прочесть.
        <br>
         <br>Мне безгранично стыдно, ведь я обещал позвонить в вечер вторника, ждал этот момент, но Его Величество Случай распорядился иначе.
         В первый день в Москве я потерял телефон, и чёрт бы с этим куском железа, но я лишился всех контактов с тобой.
         Сейчас я все делаю, полагаясь исключительно на удачу, и я даже не знаю дойдет ли до тебя моё письмо, Милена, или нет. 
         ...
         касательно всего того, что я провернул. И я готов на всё тебе ответить. Верю в то, что я заставил тебя улыбнуться) Уверен, ты сделаешь правильный выбор, который будет ближе тебе.
